...
} from '../../store/slices/product-catalogues/prod-catalogues-add-slice';
import {
  createProductCatalogue,
  getAllProductCatalogues,
  getProductCatalogueById,
  updateProductCatalogue,
} from '../../store/slices/product-catalogues/prod-catalogues-add-thunk';
...
function ProductCatalogueAdd() {
  ...
  const formik = useFormik({
    ...
    validate: (data) => {
      const errors: FormikErrors<ProductCatalogueUpdateForm> = {};
      ...
      if (catalogueId) {
        const result = await dispatch(
          updateProductCatalogue({
            ...
            parentCatalogueId: data.parentCatalogueId!,
          })
        );
        console.warn(result);
        if (result.type.includes('fulfilled')) {
          toast('Successfully updated existing catalogue!', {
            type: 'success',
          });
          navigate('./../../');
        } else {
  ...
  useEffect(() => {
    ...
    formik.setFieldValue(
      'parentCatalogueId',
      pageState.productCatalogue.parentCatalogueId!
    );
            ...
            <Stack direction="column" spacing={2}>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  value={
                    formik.values.parentCatalogueId
                      ? formik.values.parentCatalogueId
                      : ''
                  }
                  onChange={handleParentCatalogueIdChange}
                >
                  {pageState.availableCatalogues.map((x) => (
