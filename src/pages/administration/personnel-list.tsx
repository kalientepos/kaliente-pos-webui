...
import {
  hideRemoveDialog,
  removePersonnelsFromPage,
  showRemoveDialog,
} from '../../store/slices/administration/administration-slice';
import {
  getAllPersonnel,
  removePersonnel,
...
function PersonnelList() {
  ...
  const confirmRemoveDialog = async () => {
    ...
    const result = (await dispatch(removePersonnel(personnelId))) as any;
    console.log(result);
    if (result.type.includes('fulfilled')) {
      toast(`Successfully removed personnel with email: ${personnelId}`, {
        type: 'success',
      });
      dispatch(hideRemoveDialog());
    } else {
      console.log(result);
      toast(`Failed to remove personnel: ${result.payload.data.message}`, {
        type: 'error',
      });
      dispatch(hideRemoveDialog());
    }
                    ...
                    <TableCell align="center">
                      ...
                      >
                        ...
                        <Button
                          color="error"
                          variant="contained"
                          onClick={() => dispatch(showRemoveDialog(row.email))}
                        >
                          Remove
                        </Button>
                      </ButtonGroup>
