...
import {
  AddProductCataloguePageState,
  initialState,
} from './prod-catalogues-add-slice';

// #region [Thunks]
export const getAllProductCatalogues =
  createAsyncThunk<GetAllProductCataloguesResponseDto>(
...
export const getProductCatalogueById = createAsyncThunk<
  GetProductCatalogueByIdResponseDto,
  string
>('product-catalogues-add/getById', async (id, thunkAPI) => {
...
export const createProductCatalogue = createAsyncThunk<
  ProductCatalogueAddResponseDto,
  ProductCatalogueAddRequestDto
>('product-catalogues-add/create', async (dto, thunkAPI) => {
  ...
  return thunkAPI.rejectWithValue(apiResponse.data.message);
});
export const updateProductCatalogue = createAsyncThunk<
  ProductCatalogueUpdateResponseDto,
  ProductCatalogueUpdateRequestDto
>('product-catalogues-add/update', async (dto, thunkAPI) => {
...
export const removeProductCatalogue = createAsyncThunk<string, string>(
  ...
  async (id, thunkAPI) => {
    const apiResponse = await ProductCatalogueService.removeProductCatalogue(
      id
    );
    console.log(apiResponse);
    if (apiResponse && apiResponse.status != 400) {
...
const productCatalogueAddThunkBuilder = (
  builder: ActionReducerMapBuilder<any>
) => {
  builder.addCase(
    getAllProductCatalogues.pending,
    (state: AddProductCataloguePageState) => {
      console.log('[GetAllProductCatalogues] STILL PENDING...');
    }
  );

  builder.addCase(
    getAllProductCatalogues.fulfilled,
    (state: AddProductCataloguePageState, { payload }) => {
  ...
  builder.addCase(
    getProductCatalogueById.pending,
    (state: AddProductCataloguePageState) => {
  ...
  builder.addCase(
    getProductCatalogueById.fulfilled,
    (state: AddProductCataloguePageState, { payload }) => {
      ...
      state.productCatalogue = {
        ...
        parentCatalogueId: payload.product.parentCatalogueId!,
      };
    }
  );

  builder.addCase(
    getProductCatalogueById.rejected,
    (state: AddProductCataloguePageState) => {
  ...
  builder.addCase(
    removeProductCatalogue.fulfilled,
    (state: AddProductCataloguePageState, action) => {
