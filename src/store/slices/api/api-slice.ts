...
export const kalienteApi = createApi({
    ...
    tagTypes: ['Product', 'ProductCatalogue'],
    baseQuery: fetchBaseQuery({
        baseUrl: 'http://localhost:8079/',
        prepareHeaders: (headers, { getState }) => prepareApiHeaders(headers, getState)
    }),
    keepUnusedDataFor: 15,
    endpoints: (builder) => ({
        //
        addProduct: builder.mutation<BaseResponse<ProductAddResponseDto>, ProductAddRequestDto>({
            query: (reqBody) => ({
        ...
        updateProduct: builder.mutation<BaseResponse<ProductUpdateResponseDto>, ProductUpdateRequestDto>({
            query: (reqBody) => ({
        ...
        getProductById: builder.query<BaseResponse<GetProductByIdResponseDto>, string>({
        ...
        //
        addProductCatalogue: builder.mutation<BaseResponse<ProductCatalogueAddResponseDto>, ProductCatalogueAddRequestDto>({
            query: (reqBody) => ({
        ...
        updateProductCatalogue: builder.mutation<BaseResponse<ProductCatalogueUpdateResponseDto>, ProductCatalogueUpdateRequestDto>({
            query: (reqBody) => ({
        ...
        getProductCatalogueById: builder.query<BaseResponse<GetProductCatalogueByIdResponseDto>, string>({
            query: (id) => `${apiBaseRoutes.productCatalogue}/getById/${id}`,
            providesTags: (result, error, id) => [{ type: 'ProductCatalogue', id }],
        }),
        getProductCatalogues: builder.query<BaseResponse<GetAllProductCataloguesResponseDto>, void>({
            query: () => `${apiBaseRoutes.productCatalogue}/getAll`,
            providesTags: () => [{ type: 'ProductCatalogue' }],
        }),

        //
        authenticate: builder.mutation<BaseResponse<ProductCatalogueAddResponseDto>, ProductCatalogueAddRequestDto>({
            query: (reqBody) => ({
...
export const { 
    useGetProductsQuery, useGetProductByIdQuery, useAddProductMutation, useUpdateProductMutation, 
    useGetProductCataloguesQuery, useGetProductCatalogueByIdQuery, useAddProductCatalogueMutation, useUpdateProductCatalogueMutation,
