...
const ProductCatalogueService = {
  async getProductCatalogueById(
    id: string
  ): Promise<ApiResponse<GetProductCatalogueByIdResponseDto>> {
    const response = await getClient().get(
      `${baseApiUrl}/product_catalogue/getById/${id}`
    );
    console.warn(response);
    return response;
  },

  async getProductCatalogues(): Promise<
    ApiResponse<GetAllProductCataloguesResponseDto>
  > {
    const response = await getClient().get(
      `${baseApiUrl}/product_catalogue/getAll`
    );
    return response;
  },

  async addProductCatalogue(
    catalogueAddDto: ProductCatalogueAddRequestDto
  ): Promise<ApiResponse<ProductCatalogueAddResponseDto>> {
    try {
      const response = await getClient().post(
        `${baseApiUrl}/product_catalogue/add`,
        catalogueAddDto
      );
      return response;
    } catch (ex: any) {
  ...
  async updateProductCatalogue(
    catalogueUpdateDto: ProductCatalogueUpdateRequestDto
  ): Promise<ApiResponse<ProductCatalogueUpdateResponseDto>> {
    const response = await getClient().put(
      `${baseApiUrl}/product_catalogue/update`,
      catalogueUpdateDto
    );
    return response;
  },
  ...
  async removeProductCatalogue(id: string): Promise<ApiResponse<string>> {
    const response = await getClient().delete(
      `${baseApiUrl}/product_catalogue/delete/${id}`
    );
    return response;
  },
};
