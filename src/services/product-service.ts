...
const ProductService = {
    ...
    async getProductById(id: string): Promise<ApiResponse<GetProductByIdResponseDto>> {
        const response: ApiResponse<GetProductByIdResponseDto> = await getClient().get(`${baseApiUrl}/product/getById/${id}`);
    ...
    async addProduct(productAddDto: ProductAddRequestDto): Promise<ApiResponse<ProductAddResponseDto>> {
        const response = await getClient().post(`${baseApiUrl}/product/add`, productAddDto);
        return response;
    },

    async updateProduct(productUpdateDto: ProductUpdateRequestDto): Promise<ApiResponse<ProductUpdateResponseDto>> {
